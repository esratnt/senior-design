...
/*exports.listVideosByCategory = async (req, res) => {
  ...
  try {
    ...
    console.log("Video URLs:", videoUrls); // Add this log
...
exports.listVideosByCategory = async (req, res) => {
  ...
  try {
    ...
    // Ensure we're filtering to include only files directly under the specific path
    const videoUrls = files
      .filter((file) => {
        const filePath = file.name.split("/");
        if (subcategory) {
          return filePath.length === 3; // category/subcategory/file
        } else {
          return filePath.length === 2; // category/file
        }
      })
      .map((file) => {
        return {
